...
const ManagerDashboard = () => {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const [statsRes, employeesRes, leavesRes, tasksRes] = await Promise.all([
                axios.get('https://ems-5low.onrender.com/api/auth/stats', config),
                axios.get('https://ems-5low.onrender.com/api/auth/employees', config),
                axios.get('https://ems-5low.onrender.com/api/leaves', config),
                axios.get('https://ems-5low.onrender.com/api/tasks', config)
            ]);
    ...
    const handleLeaveApproval = async (leaveId, status) => {
        try {
            const token = localStorage.getItem('token');
            await axios.put(
                `https://ems-5low.onrender.com/api/leaves/${leaveId}`,
                { status },
                { headers: { Authorization: `Bearer ${token}` } }
            );
